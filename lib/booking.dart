...
import 'adapter/item_booking.dart';
import 'home.dart';
...
class BookingPage extends State<Booking> {
  ...
  getData() async {
    ...
    //Output
    var result =
        await STM().post(ctx, Str().loading, "customer/get_bookings", body);
    if (!mounted) return;
    setState(() {
      isLoaded = true;
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: titleToolbarLayout(ctx, 'My Bookings'),
        body: bodyLayout(),
      ),
    );
  }
    ...
    return Column(
      children: [
        ...
        if (position == 0 ? upcomingList.isEmpty : completedList.isEmpty)
          Expanded(
            child: STM().emptyData("No Booking Found"),
          )
      ],
    );
  }
